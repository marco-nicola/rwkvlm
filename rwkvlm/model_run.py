"""
RWKV Language Model for inference.
"""
...
print(f'\nRWKV_HEAD_QK_DIM {RWKV_HEAD_QK_DIM} '
      f'RWKV_JIT_ON {os.environ["RWKV_JIT_ON"]}\n')
...
class RWKV_RNN(MyModule):
    ...
    def LN(self, x, w):
        return layer_norm(x, (self.args.n_embd,), weight=w.weight, bias=w.bias)
    ...
    def forward(self, ctx, state, preprocess_only=False):
        with torch.no_grad():
            ...
            for i in range(args.n_layer):
                ...
                ww = w.blocks[i].att
                x = x + self.SA(
                    self.LN(x, w.blocks[i].ln1), state, i,
                    ww.time_mix_k, ww.time_mix_v, ww.time_mix_r, ww.time_first, ww.time_decay,
                    ww.key.weight, ww.value.weight, ww.receptance.weight, ww.output.weight)

                ww = w.blocks[i].ffn
                x = x + self.FF(
                    self.LN(x, w.blocks[i].ln2), state, i,
